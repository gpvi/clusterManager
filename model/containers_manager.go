...
func (c *ContainersManager) HasCluster(clusterName string) bool {
...
func (c *ContainersManager) CountByCluster(clusterName string) int {
	count := 0
	for _, node := range c.Nodes {
		if node.ClusterName == clusterName {
			count++
...
func (c *ContainersManager) CreateContainers(ctx context.Context, nodeNum int, clusterName string) error {
	...
	for id := range c.ContainersIDSet {
		...
		if conIP == "" {
			return fmt.Errorf("failed to find container ip for container %s", id)
		}
...
func (c *ContainersManager) UpdateAllContainersInfo(ctx context.Context) error {
	...
	for _, container := range containerList {
		...
		tempKey := strconv.Itoa(int(RedisContainerPort)) + "/tcp"
		hostPort := uint16(0)
		if ports, ok := inspect.NetworkSettings.Ports[tempKey]; ok && len(ports) > 0 {
			hostPort, err = utils.StringToUint16(ports[0].HostPort)
			if err != nil {
				return fmt.Errorf("failed to parse host port for container %s: %v", container.ID, err)
			}
		...
		if containerIP == "" {
			return fmt.Errorf("failed to find container ip for container %s", container.ID)
		}
		...
		c.Nodes = append(c.Nodes, &containerNode)
		c.IDToNode[container.ID] = &containerNode
		c.IPToNode[containerIP] = &containerNode
	}
	return nil
}

func (c *ContainersManager) LoadClusterContainers(ctx context.Context) error {
	if err := c.UpdateAllContainersInfo(ctx); err != nil {
		return err
	}
	c.Num = len(c.Nodes)
	return nil
